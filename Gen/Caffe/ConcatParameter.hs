...
import Control.Lens
data ConcatParameter = ConcatParameter{axis :: !(P'.Maybe P'.Int32), concat_dim :: !(P'.Maybe P'.Word32)}
...
instance P'.Mergeable ConcatParameter where
  mergeAppend (ConcatParameter x'1 x'2) (ConcatParameter y'1 y'2)
   = ConcatParameter (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
 
instance P'.Default ConcatParameter where
  defaultValue = ConcatParameter (Prelude'.Just 1) (Prelude'.Just 1)
 
instance P'.Wire ConcatParameter where
  wireSize ft' self'@(ConcatParameter x'1 x'2)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeOpt 1 5 x'1 + P'.wireSizeOpt 1 13 x'2)
  wirePut ft' self'@(ConcatParameter x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
