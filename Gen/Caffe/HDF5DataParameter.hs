...
data HDF5DataParameter = HDF5DataParameter{source :: !(P'.Maybe P'.Utf8), batch_size :: !(P'.Maybe P'.Word32),
                                           shuffle :: !(P'.Maybe P'.Bool)}
                       deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable HDF5DataParameter where
  mergeAppend (HDF5DataParameter x'1 x'2 x'3) (HDF5DataParameter y'1 y'2 y'3)
   = HDF5DataParameter (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)
 
instance P'.Default HDF5DataParameter where
  defaultValue = HDF5DataParameter P'.defaultValue P'.defaultValue (Prelude'.Just Prelude'.False)
 
instance P'.Wire HDF5DataParameter where
  wireSize ft' self'@(HDF5DataParameter x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeOpt 1 9 x'1 + P'.wireSizeOpt 1 13 x'2 + P'.wireSizeOpt 1 8 x'3)
  wirePut ft' self'@(HDF5DataParameter x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             24 -> Prelude'.fmap (\ !new'Field -> old'Self{shuffle = Prelude'.Just new'Field}) (P'.wireGet 8)
